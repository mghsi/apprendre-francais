  ...
  <div class="panel">
    <div class="actions">
      <button
        ...
        @click="runCorrection"
      >
        {{ loading ? 'Analyse en cours…' : analyseLabel }}
      </button>
      <button
        v-if="result"
        class="btn-secondary"
    ...
    <!-- Avertissement si le texte saisi a divergé du texte analysé -->
    <div v-if="result && stale" class="stale-banner">
      ✏️ Le texte a été modifié depuis l'analyse — relance l'analyse pour
...
const props = defineProps({
  ...
  analyseLabel: { type: String, default: '✨ Analyser avec Claude' },
})
...
// Reste synchronisé avec props.text via les emits update:text.
const currentText = ref('')
// Drapeaux par item
const errorFixed = ref([])
const suggestionApplied = ref([])

// Vrai si le texte a divergé entre l'analyse et le texte courant fourni
// par le parent (édition manuelle après analyse).
const stale = computed(
  () => !!result.value && props.text !== currentText.value
)

// Si le parent modifie le texte en dehors d'une correction (édition
// manuelle), on resynchronise currentText pour que les substitutions
// suivantes opèrent sur le nouveau contenu.
watch(
  () => props.text,
  (val) => {
    if (val !== currentText.value) currentText.value = val
  }
)
...
// Replaces `from` with `to` at its first occurrence, then applies the
// French elision safety-net.
...
function applyErrorFix(idx) {
  ...
  commit(spliceAndNormalise(currentText.value, e.original, e.corrected))
...
function applySuggestion(idx) {
  ...
  commit(spliceAndNormalise(currentText.value, s.original, s.suggested))
  suggestionApplied.value[idx] = true
}
